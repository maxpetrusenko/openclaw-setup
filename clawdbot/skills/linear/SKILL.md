...
- This is the **ONLY** Linear integration enabled in this workspace.
- It does **NOT** use a `linear` / `lin` CLI binary.
- It uses: `{baseDir}/scripts/linear.sh` and requires `LINEAR_API_KEY`.

**Use this skill when:** you need to list/query Linear issues/projects/teams, create issues, update status/priority, comment, assign, or generate standups.

**Do not use / do not suggest:** `lin`, `linear login`, `linear auth login`, or any other Linear CLI flow. If a user asks for that, explain it's not installed and keep using `{baseDir}/scripts/linear.sh`.

Quick sanity checks:

```bash
# OK if missing
command -v linear || true
command -v lin || true

# This is the correct entrypoint
