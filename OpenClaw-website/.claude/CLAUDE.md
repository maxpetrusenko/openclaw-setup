...
use agents.md file too and vise versa

## Agent Protocol
- Contact: Max Petrusenko (@maxpetrusenko, max.petrusenko@gmail.com).
- Workspace: `~/Projects`. Missing max's repo: clone `https://github.com/maxpetrusenko/<repo>.git`.
- 3rd-party/OSS (non-max): clone under `../Projects/`.
...
- if missing tools, use this -> https://github.com/steipete to check first
- Web: search early; quote exact errors; prefer 2024–2025 sources; fallback Firecrawl (`pnpm mcp:*`) / `mcporter`.
- skip this line now -> Oracle: run `npx -y @steipete/oracle --help` once/session before first use.
...
## Important Locations
- Blog repo: `~/Projects/medium` or Notion mcp page `Medium`
...
- skip this line -> Obsidian vault: `$HOME/Library/Mobile Documents/iCloud~md~obsidian/Documents/steipete-notes`
- skip this line -> Sparkle keys: `~/Library/CloudStorage/Dropbox/Backup/Sparkle`
...
- Model note (2025-11-23): no `gpt-5.2-high` / `grok-4.2` on Peter’s keys yet.
...
- skip this line -> Swift: use workspace helper/daemon; validate `swift build` + tests; keep concurrency attrs right.
